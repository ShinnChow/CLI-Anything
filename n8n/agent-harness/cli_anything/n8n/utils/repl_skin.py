...
# n8n harness imports these as module functions. Lazy-init to avoid
...
def output(data, as_json: bool = False) -> None:
    """Display data as JSON or formatted table/dict."""
    if as_json:
    ...
    if isinstance(data, dict):
        if "data" in data and isinstance(data["data"], list):
            ...
            if rows and isinstance(rows[0], dict):
                ...
                table_rows = [[str(r.get(h, ""))[:30] for h in headers] for r in rows]
                skin.table(headers, table_rows)
            else:
                _click.echo(_json.dumps(data, indent=2, default=str))
            if data.get("nextCursor"):
                skin.hint(f"Next cursor: {data['nextCursor']}")
        else:
            for k, v in data.items():
                skin.status(str(k), str(v)[:80])
    elif isinstance(data, list):
        if data and isinstance(data[0], dict):
            headers = list(data[0].keys())[:8]
            table_rows = [[str(r.get(h, ""))[:30] for h in headers] for r in data]
            skin.table(headers, table_rows)
        else:
            _click.echo(_json.dumps(data, indent=2, default=str))
    else:
